...
class GroundPlaneProjector:
    ...
    def segment_vegetation(self, frame):
        ...
        # Use int16 to avoid overflow and save 50% memory compared to float32
        g_16 = g_u8.astype(np.int16)
        r_16 = r_u8.astype(np.int16)
        b_16 = b_u8.astype(np.int16)
        
        exg_16 = 2 * g_16 - r_16 - b_16
        
        # Clip values to uint8 range
        exg = np.clip(exg_16, 0, 255).astype(np.uint8)
        
        # Free temporary 16-bit arrays
        del g_16, r_16, b_16, exg_16
